                    ...
                    if st.button("Update Team Status", type="primary"):
                        if new_state == "W (Working)" and not new_loc:
                            ...
                            c.execute("""UPDATE teams 
                                         SET wilaya=%s, region=%s, current_project=%s, state_code=%s, status_notes=%s, current_location_name=%s, return_to_work_date=%s, current_lat=%s, current_lon=%s 
                                         WHERE team_name=%s""", 
            ...
            with st.form("add_team_form", clear_on_submit=True):
                ...
                if st.form_submit_button("Add Team"):
                    if team_name and leader_id:
                        ...
                        c = conn.cursor()
                        try:
                            # FIXED SQL SYNTAX FOR POSTGRES (%s)
                            c.execute('''INSERT INTO teams 
                                        ...
                                        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, 'Available', 'S', %s)''',
            ...
            with st.expander("✏️ Edit or Delete Team"):
                ...
                if not df_edit.empty:
                    ...
                    if update_btn:
                        ...
                        conn = get_connection(); c = conn.cursor()
                        c.execute("""UPDATE teams 
                                     SET leader_id=%s, leader_name=%s, skills=%s, wilaya=%s, home_location_name=%s, home_lat=%s, home_lon=%s, start_date=%s 
                                     WHERE team_name=%s""", 
